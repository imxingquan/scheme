...
#|
https://htdp.org/2003-09-26/Solutions/perm-xmas.html
习题17.6.5 
...
;;Templete
#|
这个函数读入名字的表L 和一个排列的表,返回在所有位置上都和L 不同的排列的表。
(define (non-same names arrangements)
  (cond
    [(empty? arrangements) ...]
    [else
       (first arrangements)
       (non-same names (rest list-of-names)]))
|#
...
;;randome-pick : non-empty-list-of-list-of-names -> list-of-names
(define (random-pick arrangements)
...
------------------------------------------------------
;Templete:
(define (any-math name arrangement)
  (cond
   [(empty? name)...]
   [else ... (first name)...(first arrangement)...
         ... (any-math (rest name) (rest arrangement))...]))
...
many arrangements are in a list of arrangeents.
----------------------------------------------------
;Templete:
...
;;consumes a list of arrangements and return
;;how many arrangements are in the list
(define (size-of arrangements)
  (cond
...
#|We need a helper function that picks the nth
arrangement in a list of arrangements
-----------------------------------------------
;Templete:
...
;;arrangement in the list at position n (position starts at 0)
(define (pick-nth n arrangements)
   (cond
...
(define (add-at-beginning a-letter lo-words)
  (cond
    [(empty? lo-words) empty]
    [else
     (cons (cons a-letter (first lo-words))
